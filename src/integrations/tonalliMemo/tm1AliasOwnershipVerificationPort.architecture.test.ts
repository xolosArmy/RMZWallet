...
describe('TM1 alias ownership verification port isolation', () => {
  ...
  test('public factory binds trusted transport and rejects fetch/endpointUrl', () => {
    ...
    expect(portRuntime).toContain('String.prototype.concat')
    expect(portRuntime).not.toContain('Array.prototype.join')
    expect(portRuntime).not.toContain('Array.prototype.push')
    ...
    expect(portRuntime).toContain('function settleCancel')
    expect(portRuntime).toContain('Promise.resolve(cancelStreamReader')
    expect(portRuntime).not.toMatch(/void\s+cancelStreamReader/)
    expect(portRuntime).toContain("Object.getOwnPropertyDescriptor(Response.prototype, 'body')")
    expect(portRuntime).toContain('getResponseBody')
    ...
    expect(bodyRead).not.toContain('pushValue')
    expect(bodyRead).not.toContain('joinStrings')
    expect(bodyRead).not.toMatch(/\bparts\b/)
    expect(bodyRead).not.toMatch(/\[[0-9]+\]\s*=/)
    expect(bodyRead).toContain('concatStrings')
    expect(bodyRead).toContain('settleCancel(reader)')
    expect(bodyRead).not.toContain('void cancelStreamReader')
    expect(bodyRead).toContain('getResponseBody(response)')
    expect(bodyRead).not.toMatch(/response\.body/)
    ...
    expect(portRuntime).toContain('nowMs()')
    expect(portRuntime).toContain('AbortController.prototype.abort')
    expect(observe).not.toMatch(/\bcontroller\.abort\s*\(/)
    expect(observe).toContain('abortController(controller)')
    expect(portRuntime).toContain('getAbortSignal = Function.prototype.call.bind(')
    expect(observe).not.toMatch(/\bcontroller\.signal\b/)
    expect(observe).toContain('getAbortSignal(controller)')
    expect(portRuntime).toContain('EventTarget.prototype.addEventListener')
    expect(portRuntime).toContain('EventTarget.prototype.removeEventListener')
    expect(observe).not.toMatch(/callerSignal\??\.addEventListener/)
    expect(observe).toContain('registerAbortListener(callerSignal, onAbort)')
    expect(aliasRuntime).toContain('runWithIsolatedStringDecoder')
    ...
    expect(aliasRuntime).toContain('objectGetPrototypeOf')
    expect(aliasRuntime).toContain('objectSetPrototypeOf')
    expect(aliasRuntime).toContain('capturedGlobalParseInt')
    expect(aliasRuntime).toContain('tamperedProtoIndices')
